...
if (!class_exists('<%= classPrefix %>PluginAdmin')) {
    class <%= classPrefix %>PluginAdmin {
        private $name;
        private $description;
        private $slug;
        private $version;
        ...
        public function checkPermission(WP_REST_Request $request) {
            if (!wp_verify_nonce($request->get_header('X-WP-Nonce'), 'wp_rest')) {
                return new WP_Error('rest_forbidden', __('Scrape nonce check failed. Please try again.'), array('status' => 403));
            }

            if (!current_user_can($this->capability)) {
                return new WP_Error('rest_forbidden', __('You do not have permission to perform this action.'), array('status' => 403));
            }
        ...
        public function updatePreferences(&$result) {
            ...
            if (isset($_REQUEST['option'])) {
                ...
                update_option($this->getOptionName(sanitize_key($option['name'])), sanitize_text_field($option['value']));
