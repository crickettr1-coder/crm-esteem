...
class Leads extends Security_Controller {
    ...
    private $duplicate_leads_skipped = 0;
    ...
    private function _save_a_row_of_excel_data($row_data) {
        ...
        //Do not import the same lead more than once. Email is the safest
        //identifier available in the import format. This also catches
        //duplicate rows within the same file because each row is saved
        //before the next one is processed.
        $contact_email = trim(get_array_value($lead_contact_data, "email"));
        if ($contact_email) {
            $existing_lead = $this->Users_model->get_one_where(array(
                "email" => $contact_email,
                "user_type" => "lead",
                "deleted" => 0
            ));
        ...
        foreach ($row_data as $column_index => $value) {
            ...
            if ($column_name == "name") {
                ...
                if ($source_id) {
                    ...
                    $lead_data["lead_source_id"] = $saved_source_id;
